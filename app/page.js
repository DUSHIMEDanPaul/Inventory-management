...
export default function Home() {
  ...
  const [itemName, setItemName] = useState('');  // Fixed initialization to an empty string
  ...
  const updateInventory = async () => {
    ...
    docs.forEach((doc) => {
      inventoryList.push({
        name: doc.id,  // Fixed to use the correct field name
        ...doc.data()
      });
    });
      ...
      <Modal open={open} onClose={handleClose}>
        <Box position='absolute' top='50%' left='50%' sx={{transform:'translate(-50%, -50%)'}} width={400} bgcolor={'white'} border={'2px solid #000'} boxShadow={24} p={4} display={'flex'} flexDirection={'column'} gap={3}>
          ...
          <Stack width={'100%'} flexDirection={'row'} spacing={2}>
            ...
            />
            <Button variant='outlined' position='absolute' top='70%' left='90%' sx={{transform:'translate(30%, -6.5%)'}} p={5} gap={4}onClick={()=>{
              AddItem(itemName);
              setItemName('');
              handleClose();
            }}
